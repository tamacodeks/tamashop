<form class="form-horizontal" name="frmInvoiceSetting" id="frmInvoiceSetting" action="{{ secure_url('invoice-settings/update/'.$setting->id) }}" method="POST">
    {{ csrf_field() }}
        ...
        <div class="col-md-8">
            <select class="form-control select2" name="country_id" id="country_id">
                ...
                @forelse($countries as $country)
                    <option value="{{ $country->id }}" @if($setting->country_id == $country->id) selected @endif>{{ $country->nice_name }}</option>
                @empty
                @endforelse
            </select>
        </div>
    ...
    <div class="form-group">
        ...
        <div class="col-md-8">
            <input type="text" name="commission" id="commission" class="form-control" value="{{ $setting->commission }}">
        </div>
    </div>
    <div class="form-group">
